...
var crop_limit = "<p>Crop Acreage Limits are used to limit the acreage of specific crops or group of crops. They can be used whether you are planning by acres or fields.</p>" +
	"<p>Crop Acreage Limits help you formulate and evaluate diversified cropping strategies. Since PlantingProfits® focuses on maximizing estimated profit, it will create crop strategies that contain as many acres as possible of the most profitable crop (among other considerations), so Crop Acreage Limits are used to generate strategies that include minimum and maximum acres of a particular crop or group of crops that you would like to specify.</p>" +
...
var crop_limit_msg1 = "<div id='more_div' style='display:none' class='more_div12'><p>Crop Acreage Limits are very useful but there are a couple of things to bear in mind.</p>" +
	"<p>First, if you specify a minimum acreage limit but do not provide enough of the required resources to meet the minimum, PlantingProfits® will not be able to generate the most profitable combination of crops and acreage/fields. In this case you will need to either: 1) decrease the minimum acreage limit or 2) increase the limiting resource.</p>" +
	...
	" You can set Minimum and Maximum crop acreage limits on a given crop or group of crops. This will bracket the acreage of the crop or group of crops between the minimum and maximum. You can set Crop Acreage Limits using percentages or an amount of land.</p>" +
	"<p>You can set Minimum <b>and</b> Maximum crop acreage limits on a given crop or group of crops. This will bracket the acreage of the crop or group of crops between the minimum and maximum. You can set Crop Acreage Limits using percentages or an amount of land." +
	"If you set the Minimum and Maximum crop acreage limits equal then PlantingProfits® will generate the most profitable combination of crops and acreage with exactly the specified acreage. This should only be used when planning by Acres. If you set the Minimum and Maximum crop acreage limits equal when planning by Fields, PlantingProfits® will likely not be able to assign the crop or group of crops to a set of fields that exactly matches the acreage limits.</p></div>";
...
var featuredScenario = "<p>If you have created scenarios you can select one to include in the report.  You can only include one scenario in the report.</p>"
...
var ReturnOnWorkingCapital="<p>Return on Working Capital is a measure of the efficiency of a crop to generate a dollar of estimated income for a dollar of Working Capital used.<p></p> It is the Estimated Income divided by the Variable production costs for a given crop. </p>";

var RatingForReturnOnWorkingCapital="<p> Return on Working Capital Index rating is as follows:<p>"+
    "<p>Green : Est. Income / Working Capital is greater than or equal to 0.90</p>"+
    "<p>Yellow : Est. Income / Working Capital is less than 0.9 and greater than or equal to 0.5</p>"+
    "<p>Red : Est. Income / Working Capital is less than 0.5</p>";
...
var diversification = "<p>This is the percentage of a strategy’s Estimated Income generated by the one crop with the greatest contribution to Estimated Income.</p>";

var forwardSales = "<p>This is the percentage of a strategy’s Estimated Income forward sold to reduce market risk.</p>";

var returnOnWorkingCapital = "<p>Return on Working Capital is a measure of the efficiency of a strategy to generate a dollar of estimated income for a dollar of Working Capital used.\n" +
	"<p> It is the Estimated Income divided by the Working Capital for a given strategy.</p> " +
	"</p>";

var acreageUnderConservationPractice = "<p>Acreage Under Conservation Practices is the percentage of crop acreage under conservation practices.</p>";
...
function callDiv_Show13(obj) {
	...
	} else if(id == "featuredScenario") {
        contentArray.push(featuredScenario);
	...
	}else if(id=="ReturnOnWorkingCapital"){
        contentArray.push(ReturnOnWorkingCapital);}
    else if(id=="RatingForReturnOnWorkingCapital"){
        contentArray.push(RatingForReturnOnWorkingCapital);}
    else if(id=="diversification"){
        contentArray.push(diversification);}
    else if(id=="forwardSales"){
        contentArray.push(forwardSales);}
    else if(id=="returnOnWorkingCapital"){
        contentArray.push(returnOnWorkingCapital);}
    else if(id=="acreageUnderConservationPractice"){
        contentArray.push(acreageUnderConservationPractice);}
