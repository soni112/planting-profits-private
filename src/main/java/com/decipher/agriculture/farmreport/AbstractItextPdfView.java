...
public abstract class AbstractItextPdfView extends AbstractView {
	...
	protected Document newDocument() {
		return new Document(PageSize.A4, 10, 10, 10, 30);
	}
	...
	protected final void pdfErrorHandler(Document document, String[] errors) {
		for (int i = 0; i < errors.length; i++)
			try {
				document.add(new Paragraph(errors[i]));
			} catch (DocumentException e) {
				logger.error("Error while adding error in pdf");
			}
	}
